...
class BinaryTree{
		...
		void insert(int data,Node * &node,Node *parent);
            //@not used for now
...
void BinaryTree::insert(int data,Node * &node,Node *parent)
{
	if(node == NULL) 
      {
            //@insert node here
            node = new Node(data);
            if ( !parent) return;
            if( parent->data < data) parent->setR(node);
            else parent->setL(node);
      }
	else if(node->data < data) insert(data,node->right,node);
